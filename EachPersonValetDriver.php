...
class EachPersonValetDriver extends BasicValetDriver
{
    ...
    {
        return $this->getAngularBuildPath($sitePath) !== null || file_exists($sitePath . self::WORDPRESS_DIRECTORY . '/wp-config.php');
    }
    ...
    public function isStaticFile(string $sitePath, string $siteName, string $uri)
    {
        if (str_starts_with($uri, self::ANGULAR_DIRECTORY)) {
            $sitePath = $this->getAngularBuildPath($sitePath);
     ...
     */
    private function getAngularBuildPath(string $sitePath): ?string
    {
        ...
        $angularJsonPath = $angularDir . '/angular.json';

        if (!file_exists($angularJsonPath)) {
            return null;
        }

        $angularJson = json_decode(file_get_contents($angularJsonPath), true);

        if (json_last_error() !== JSON_ERROR_NONE || !isset($angularJson['projects'])) {
            return null;
        }

        $projectName = $angularJson['defaultProject'] ?? array_key_first($angularJson['projects']);
